...
int main(int argc, char** argv) {
	...
	while(1){
		...
		//enc server
			...
			char message_2_encrypt[2049];
			int total_message_recieved = 0;
			int max_message = 2048;

			//clientwill send the length of the key and plaintext first
			...
			while (received < length){
				ssize_t n = recv(communication_socket, plaintext + received, length - received, 0);
				if (n <= 0){
					fprintf(stderr, "error when reading plaintext in enc server");
				}
				received += n;
			}
			plaintext[length] = '\0';
			...
			for(int i = 0; i < length; i++){
				plaintext[i] = encrypt_char(plaintext[i], key[i]);
			}

			int total_len = length + 1;
			char *buff = malloc(total_len);

			memcpy(buff, plaintext, length);
			buff[length] = '\n';
